...
- **Home** - landing screen: the whole hero block (title, subtitle, lede)
  types itself in as one continuous sequence, looping forever
  (`js/core/typewriter.js`'s `loopTypeSequence`), a faint evacuation-rate choropleth
  (`assets/evac-rate-bg.svg`, generated by `scripts/generate_bg_map.py`)
  is revealed in a soft circle that follows the cursor
  (`js/core/spotlight.js`) against an otherwise pure-black background, the
  navigation grid links to all seven pages, and a footer credits the lab,
  author, and advisor. Clicking any card/nav link fades the current page
  out before swapping to the next, so navigating reads as a real page
  jump rather than content changing in place.
...
- `js/app.js` - entry point: wires up the router, the homepage typewriter
  and spotlight background, and lazily initializes the Page 3 map and the
  Page 4 profile cards the first time each is opened
...
- `js/core/typewriter.js` - `loopTypeSequence()` types a list of
  {element, text} segments one after another (title, subtitle, lede),
  holds, deletes the whole sequence, and repeats forever; re-run every time
  the home page is (re)entered. `loopType()` is a one-segment convenience
  wrapper around it.
- `js/core/spotlight.js` - cursor-follow reveal for the homepage
  background art (`assets/evac-rate-bg.svg`)
...
- `assets/logo.png` - site logo, used in the top nav and on the homepage
- `assets/evac-rate-bg.svg` - decorative homepage background (evacuation
  rate by block group), not the interactive map
...
- `scripts/generate_bg_map.py` - regenerates `assets/evac-rate-bg.svg` from
  `data/blockgroups.geojson`; re-run after `prepare_data.py` if the
  underlying evacuation-rate data changes
